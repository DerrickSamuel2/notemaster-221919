...
# IMPORTANT: even if Postgres is already running, we still want to ensure migrations/schema are applied.
if sudo -u postgres ${PG_BIN}/pg_isready -p ${DB_PORT} > /dev/null 2>&1; then
    ...
    echo ""
    echo "Ensuring database/user exist (best-effort) and applying migrations..."

    # Best-effort: ensure database exists. If it already exists, this is harmless.
    sudo -u postgres ${PG_BIN}/createdb -p ${DB_PORT} ${DB_NAME} 2>/dev/null || echo "Database might already exist"

    # Best-effort: ensure role exists & has privileges.
...
EOF
    ...
    # Save connection command to a file (useful if this is the first run and file didn't exist yet)
    echo "psql postgresql://${DB_USER}:${DB_PASSWORD}@localhost:${DB_PORT}/${DB_NAME}" > db_connection.txt
    echo "Connection string saved to db_connection.txt"

    # Save environment variables to a file (best-effort; may fail if directory missing)
    mkdir -p db_visualizer >/dev/null 2>&1 || true
...
EOF
    ...
    echo ""
    echo "Done (Postgres was already running; migrations ensured)."
    exit 0
fi
